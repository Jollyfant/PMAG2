...
function calculateIterationBootstrap(one, two, grid, data) {
    ...
    var X10_b = math.subset(data['X10'], math.index(math.range(0, math.size(data['X10'])[0]), idx1))
    ...
    var mhat10_b_flat = math.flatten(math.matrix(mhat10_b_normalized))
    mhat10_b_flat = mhat10_b_flat._data

    // Mhat10_b = form_Mhat(mhat10_b)
    const Mhat10_b = form_Mhat(mhat10_b_flat)

    // Ghat10_b = form_Ghat(X10_b, Mhat10_b)
    const Ghat10_b = form_Ghat(X10_b, Mhat10_b)

    var idx2 = Array.from({length: data['n2']}, () => Math.floor(Math.random() * data['n2']))

    // X20_b = np.asarray(X20[:, idx2])
    const X20_b = math.subset(data['X20'], math.index(math.range(0, math.size(data['X20'])[0]), idx2))
    ...
    const mhat20_b_flat = math.flatten(math.matrix(mhat20_b_normalized))

    // Mhat20_b = form_Mhat(mhat20_b)
    const Mhat20_b = form_Mhat(mhat20_b_flat._data)

    // Ghat20_b = form_Ghat(X20_b, Mhat20_b)
    const Ghat20_b = form_Ghat(X20_b, Mhat20_b)
    try {
        ...
        var T_b = math.multiply(math.transpose(data['mhat0']), math.multiply(Ahat_b, data['mhat0']))[0][0]
...
function calculateConfidenceRegion(T_b) {
    // console.log('calculateConfidenceRegion T_b', T_b)
    const sortedT = math.sort(T_b)
    const qT = Math.ceil((1 - 0.05) * sortedT.length) - 1;
    return sortedT[qT]
}

function calculateTestedDirection(Ahat, DEC, INC) {
    let direction = new Direction(Number(DEC), Number(INC)).toCartesian()
    const m = math.transpose([direction.x, direction.y, direction.z])
    const resultMatrix = math.multiply(m, Ahat);
    return math.multiply(resultMatrix, m);
}

function calculateLminC(Lmin_b) {
    const Lmin_b_toSort = Lmin_b.slice()
...
function unfoldInclinationOnly(vectors, type, iteration) {
    ...
    function inverseVarianceOfUnfoldedDirections(vectors, unfoldingPercentage) {
        ...
        var tiltsDecInc = math.matrix(tilts)._data.map(x => new Coordinates(x[0], x[1], x[2]).toVector(Direction));
        var inclinations = tiltsDecInc.map((vector) => {
            return vector.inc
        })
...
function find_CMDT_CR(Ahat, Tc, mhat12) {
    ...
    for (var i = 0; i < 201; i++) {
        ...
        V_matrix = math.multiply(V_matrix, -1)
...
function simulateCTMD(one, two, grid, containers, selectedCollections) {
  ...
  two = two.filter(x => !x.rejected);
    ...
    var X1 = [xOne, yOne, zOne]
    var X2 = [xTwo, yTwo, zTwo]
    ...
    const n1 = X1[0].length
    const n2 = X2[0].length
    const n = n1 + n2
    ...
    const Ghat1 = form_Ghat(X1, Mhat1)

    const Mhat2 = form_Mhat(mhat2)
    const Ghat2 = form_Ghat(X2, Mhat2)
    ...
    mhat0 = math.multiply(mhat0, -1)

    const Q1 = form_Q(mhat0, mhat1)
    var X10 = math.multiply(Q1, X1)
    X10 = X10._data

    const Q2 = form_Q(mhat0, mhat2)
    var X20 = math.multiply(Q2, X2)
    X20 = X20._data
    ...
    (next = function () {
        ...
        if (iteration > NUMBER_OF_BOOTSTRAPS) {
            ...
            // return plotBootstrapCTMD(NUMBER_OF_BOOTSTRAPS, finalResult, containers, selectedCollections)
            return plotBootstrapCTMD(NUMBER_OF_BOOTSTRAPS, finalResult, containers, selectedCollections)
        }
        result = calculateIterationBootstrap(one, two, grid, data)

        if (result.CMDTdata.Lmin_b !== null) {
            ...
            Lmin_b[iteration] = result.CMDTdata.Lmin_b
            T_b[iteration] = result.CMDTdata.T_b
            iteration++
        }
...
function calculateCMDTWithDirection(T_b, Ahat, X1c_di, X2c_di, names, mhat12) {
    ...
    const transpose = math.transpose(mCI)
    var DIc = []
    transpose.forEach(vector => {
        const vectorData = {
            "x": vector[0],
            "y": vector[1],
            "z": vector[2]
        }
        DIc.push(literalToCoordinates(vectorData).toVector(Direction))
    })
    const DIc_di = DIc.map(row => new Object({
        x: row.dec,
        y: projectInclination(row.inc),
        inc: row.inc
    }))
...
function eqAreaProjectionMean() {
  ...
  selectedCollections.forEach(function(site) {
    ...
    var A95Ellipse = getConfidenceEllipse(statistics.pole.confidence);
        ...
        var a95ellipse
        if (A95_CONFIDENCE) {
            a95ellipse = transformEllipse(A95Ellipse, statistics.dir);
        } else {
    ...
    if(document.getElementById("show-samples-mean").checked) {
      ...
      dataSeries.push({
        ...
        var icon
        if (site.doi) {
            icon = "<span class='text-success'><i class='fas fa-id-card'></i></span>";
        } else {
...
function eqAreaProjectionStatistics(vgp) {
    /*
  ...
  var baseSite = new Site(0, 0);
    ...
    var convertedCompsCutOff = doCutoff(convertedComps);
    var poleStatistics = getStatisticalParameters(convertedCompsCutOff.components);
    ...
    var pole = baseSite.poleFrom(new Direction(direction.dec, direction.inc));
        ...
        if (!vgp || !component.latitude || !component.longitude) {
            ...
            baseSite = new Site(component.longitude, component.latitude)
    ...
    console.log('A95Ellipse', A95Ellipse.map(prepareDirectionData))
    console.log('A95EllipsePole', A95EllipsePole)

    var dataMean = []
    if (vgp) {
        dataMean = new Array(poleStatistics.dir.mean).map(prepareDirectionData)
    } else {
        dataMean = [{
            "x": 0,
            "y": 90,
    ...
    var poleSeries = [{
        ...
        "marker": {
            ...
            "lineWidth": 1,
            "fillColor": vgp ? (poleStatistics.dir.mean.inc < 0 ? HIGHCHARTS_WHITE : HIGHCHARTS_GREEN) : (statistics.dir.mean.inc < 0 ? HIGHCHARTS_WHITE : HIGHCHARTS_GREEN)
        },
        "data": dataMean
    }, {
        "name": "Confidence Ellipse",
        ...
        "type": "line",
        "color": HIGHCHARTS_RED,
        "data": vgp ? A95EllipsePole : A95Ellipse.map(prepareDirectionData),
        "enableMouseTracking": false,
        "marker": {
            "enabled": false
        }
    ...
    if (document.getElementById("enable-deenen").checked) {
        ...
        var markPole = (poleStatistics.pole.confidenceMin < poleStatistics.pole.confidence && poleStatistics.pole.confidence < poleStatistics.pole.confidenceMax) ? "✓" : "×";
...
function selectBootstrap() {
    ...
    switch (typeBootstrap) {
        ...
        case 'inclination-only':
            ...
            break;
        case 'both':
            bootstrapFoldtest(true)
            break;
    }
}

function bootstrapFoldtest(both = false) {
    ...
    // Get the components for each site (no cutoff applied)
    const cutoffCollectionsG = collections.map(function (collection) {
        return collection.components.map(x => x.inReferenceCoordinates("geographic"));
    });

    // The same for tectonic coordinates
    const cutoffCollectionsT = collections.map(function (collection) {
        return collection.components.map(x => x.inReferenceCoordinates("tectonic"));
    });
    ...
    // Combine all geographic components to a single array
    const vectors = [].concat(...cutoffCollectionsG);
...
function calculateInclinationOnly() {
    ...
    var directions = math.transpose(coordinates)._data.map(x => new Coordinates(x[0], x[1], x[2]).toVector(Direction));

    var inclinations = []

    for (let d of directions) {
        inclinations.push(d.inc)
    }
...
function plotUnfoldedData() {
    ...
    // Combine all geographic components to a single array
    var dirs = [].concat(...cutoffCollectionsG);
...
function plotHistograms(Lmin_b, B, Lmin, Lmin_c) {
    var binCount = Math.ceil(Math.sqrt(B))
    var binWidth = (Math.max(...Lmin_b) - Math.min(...Lmin_b)) / binCount
    var binFrequencies = []
    for (var i = 0; i <= binCount; i++) {
        binFrequencies[i] = 0
    }

    Lmin_b.forEach(value => {
        var binIndex = Math.floor((value - Math.min(...Lmin_b)) / binWidth)
...
function plotBootstrapCTMD(NUMBER_OF_BOOTSTRAPS, result, containers, selectedCollections) {
    ...
    const transpose = math.transpose(result.CMDTData.X1)
    var X1c = []
    transpose.forEach(vector => {
        const vectorData = {
            "x": vector[0],
        ...
        X1c.push(literalToCoordinates(vectorData).toVector(Direction))
    })
    const X1c_di = X1c.map(row => new Object({
        x: row.dec,
        y: projectInclination(row.inc),
        inc: row.inc
    }))
    ...
    const transpose2 = math.transpose(result.CMDTData.X2)
    // const transpose2 = math.transpose(math.multiply(result.CMDTData.X2, -1))
    var X2c = []
    transpose2.forEach(vector => {
        const vectorData = {
            "x": vector[0],
            "y": vector[1],
            "z": vector[2]
        }
        X2c.push(literalToCoordinates(vectorData).toVector(Direction))
    })
    const X2c_di = X2c.map(row => new Object({
        x: row.dec,
        y: projectInclination(row.inc),
        inc: row.inc
    }))
    ...
    var names = {
        "one": selectedCollections[0].name,
        "two": selectedCollections[1].name
    }
...
function plotFoldtestInclinationOnly(untilt, savedBootstraps, type) {
    ...
    $("#foldtest-full-container").css("display", "none");
    $("#foldtest-io-full-container").css("display", "block");

    var typeBootstrap = document.getElementById("select-foldtest").value
    if (typeBootstrap === "both") {
        $("#foldtest-full-container").css("display", "block");
        $("#foldtest-io-full-container").css("display", "block");
    }
    ...
    new Highcharts.chart(CHART_CONTAINER, {
        ...
        "credits": {
            "enabled": ENABLE_CREDITS,
            "text": "Paleomagnetism.org [Foldtest Module] - after Tauxe et al., 2010",
            "href": ""
        },
        "tooltip": {
            "enabled": true,
...
function plotFoldtestCDF(untilt, savedBootstraps) {
    ...
    $("#foldtest-full-container").css("display", "block");
    $("#foldtest-io-full-container").css("display", "none");
